...
import { Item, LogAction } from '../types';
...
export const Inventory: React.FC = () => {
  const { state, borrowItem, addItem, editItem, deleteItem } = useInventory();
  ...
  const itemHasOutstandingLoans = useMemo(() => {
    ...
    return state.logs.some(
        log => log.itemId === itemToDelete.id && log.action === LogAction.BORROW && !returnedLogIds.has(log.id)
    );
